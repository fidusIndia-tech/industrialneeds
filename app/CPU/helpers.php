...
class Helpers
{
    ...
    public static function update_shipping_cost_by_country($cart_id){
        $cart = Cart::findOrFail($cart_id);
        $cart_list = Cart::where('cart_group_id', $cart->cart_group_id)->get();
        $total_shipping_cost = 0;
        foreach($cart_list as $item){
            $shipping_country = ShippingCostByCountry::where(['country_id'=> auth('customer')->user()->country_id,'product_id' => $item->product_id])->first();
            if(!is_null($shipping_country)){
                $total_shipping_cost = $total_shipping_cost+($shipping_country->shipping_cost*$item->quantity);
            }
        }
        $cart_ship = CartShipping::where('cart_group_id', $cart->cart_group_id)->first();
        if(!is_null($cart_ship)){
               $cart_ship->shipping_cost = $total_shipping_cost;
               $cart_ship->save();
        }else{
            CartShipping::create(['cart_group_id'=>$cart->cart_group_id,'shipping_method_id'=>1,'shipping_cost'=> $total_shipping_cost]);
        }
    }
    ...
    {
        $x = ['kg', 'pc', 'gms', 'ltrs', 'Nos', 'Pair', 'meter', 'Set', 'Packet', 'Bundle', 'Role'];
        return $x;
    }

    public static function remove_invalid_charcaters($str)
    {
        return str_ireplace(['\'', '"', ',', ';', '<', '>', '?'], ' ', ucwords($str));
    }
