                ...
                <form class="product-form" action="{{route('admin.product.update',$product->id)}}" method="post"
                    ...
                    <div class="row mt-5">
                        <div class="col-md-4 mb-2">
                            <label class="control-label">{{ \App\CPU\translate('Country') }} </label>
                        </div>
                        <div class="col-md-4 mb-2">
                            <label class="control-label">{{ \App\CPU\translate('duration') }} </label>
                        </div>
                        <div class="col-md-4 mb-2">
                            <label class="control-label">{{ \App\CPU\translate('shipping_cost') }} </label>
                        </div>
                        @foreach($countries as $country)
                        @php($shipping_cost = \App\ShippingCostByCountry::where(['country_id'=>$country->id, 'product_id'=>$product->id])->first())
                        <input type="hidden" name="country[{{$country->id}}][id]" value="{{ @$shipping_cost->id ? base64_encode(@$shipping_cost->id) : null}}"/>
                        <div class="col-md-4 mb-2">
                            <label class="control-label">{{ \App\CPU\translate($country->name) }} </label>
                        </div>
                        <div class="col-md-4 mb-2">
                            <input type="text"placeholder="5 to 6 days" value="{{@$shipping_cost->duration}}" name="country[{{$country->id}}][duration]" class="form-control">
                        </div>
                        <div class="col-md-4 mb-2">
                            <input type="number" min="0" step="1" value="{{ \App\CPU\BackEndHelper::usd_to_currency(@$shipping_cost->shipping_cost)}}" placeholder="{{ \App\CPU\translate('shipping_cost') }}" name="country[{{$country->id}}][cost]" class="form-control">
                        </div>
                        @endforeach
                    </div>
